...
export default function RepoCards({ findings }: { findings: RepoFinding[] }) {
  ...
  return (
    <div>
      <h2 className="text-sm font-medium text-gray-400 mb-3">Repositories</h2>
      <div className="grid grid-cols-2 gap-4">
        {findings.map((g) => (
          <div key={g.repo} className="bg-[#111118] rounded-xl border border-white/10 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-medium text-white">{g.repo}</span>
            </div>
            <div className="flex gap-4 text-sm text-gray-500">
              <span>
                ⭐ <span className="text-white font-medium">{fmt(g.stars)}</span>{" "}
                <Delta n={g.starsDelta} />
              </span>
              <span>
                🍴 <span className="text-white font-medium">{fmt(g.forks)}</span>{" "}
